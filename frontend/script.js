const API_URL = 'http://localhost/task-management-api/api';
let currentPage = 1;
let totalPages = 1;

function checkAuth() {
    if (!localStorage.getItem('loggedIn') && !window.location.pathname.includes('login.html') && !window.location.pathname.includes('register.html')) {
...
async function updateTask(id) {
    ...
    const status = prompt('New status (pending/done)');
